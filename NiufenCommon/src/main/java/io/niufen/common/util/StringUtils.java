...
public class StringUtils {
    ...
    public static boolean startWithAny(CharSequence cs, CharSequence... prefixes) {
        if (isEmpty(cs) || ArrayUtils.isEmpty(prefixes)) {
            return false;
        }
    ...
    public static boolean endWithAny(CharSequence cs, CharSequence... suffixes) {
        if (isEmpty(cs) || ArrayUtils.isEmpty(suffixes)) {
            return false;
        }
    ...
    public static boolean containsAny(CharSequence cs, char... csArgs) {
        if (isNotEmpty(cs) && ArrayUtils.isNotEmpty(csArgs)) {
    ...
    public static boolean containsOnly(CharSequence cs, char... csArgs) {
        if (isEmpty(cs) || ArrayUtils.isEmpty(csArgs)) {
            return false;
        }
        for (int i = 0; i < cs.length(); i++) {
            if (!ArrayUtils.contains(csArgs, cs.charAt(i))) {
                return false;
            }
        }
        return true;
    ...
    public static String removeAll(CharSequence str, char... charArgs) {
        if (isEmpty(str) || ArrayUtils.isEmpty(charArgs)) {
            return str(str);
        }
        ...
        for (int i = 0; i < len; i++) {
            c = str.charAt(i);
            if (!ArrayUtils.contains(charArgs, c)) {
