...
public class TypeUtil {
    ...
    public static Type getParamType(Method method, int index) {
        ...
        if (ArrayUtil.isEmpty(types) || types.length <= index) {
            return null;
        }
        return types[index];
    }
    ...
    public static Class<?> getParamClass(Method method, int index) {
        ...
        if (ArrayUtil.isEmpty(classes) || classes.length <= index) {
            return null;
        }
...
//        final Map<TypeVariable<?>,Type> tableMap = new TableMap<>(typeVars,actualTypeArguments);
        ...
        final List<TypeVariable<?>> keyList = ListUtil.newList(size);
        final List<Type> valueList = ListUtil.newList(size);
