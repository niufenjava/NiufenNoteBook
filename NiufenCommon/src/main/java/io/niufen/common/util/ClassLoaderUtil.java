...
public class ClassLoaderUtil {
    ...
    static {
        List<Class<?>> primitiveTypes = ListUtil.newList(32);
        // 加入原始类型
    ...
    public static Class<?> loadClass(String name, ClassLoader classLoader, boolean isInitialized) throws UtilException {
        ...
        if (StrUtil.endWith(name, ARRAY_SUFFIX)) {
            // 对象数组 "java.lang.String[]" 风格
            final String elementClassName = StrUtil.removeSuffix(name, ARRAY_SUFFIX);
            // 递归
            final Class<?> elementClass = loadClass(elementClassName, classLoader, isInitialized);
            clazz = Array.newInstance(elementClass, 0).getClass();
        } else if (StrUtil.startWith(name, NON_PRIMITIVE_ARRAY_PREFIX) && StrUtil.endWith(name, StringConstants.Mark.SEMICOLON)) {
            // 内部非原始类型类名前缀 "[L"
            // "[Ljava.lang.String;" 风格
            final String elementName = StrUtil.sub(name, NON_PRIMITIVE_ARRAY_PREFIX.length(), name.length() - 1);
            final Class<?> elementClass = loadClass(elementName, classLoader, isInitialized);
            clazz = Array.newInstance(elementClass, 0).getClass();
        } else if (StrUtil.startWith(name, INTERNAL_ARRAY_PREFIX)) {
            // "[[I" 或 "[[Ljava.lang.String;" 风格
            final String elementName = StrUtil.subSuf(name, INTERNAL_ARRAY_PREFIX.length());
            final Class<?> elementClass = loadClass(elementName, classLoader, isInitialized);
            clazz = Array.newInstance(elementClass, 0).getClass();
        } else {
            // 加载普通类
