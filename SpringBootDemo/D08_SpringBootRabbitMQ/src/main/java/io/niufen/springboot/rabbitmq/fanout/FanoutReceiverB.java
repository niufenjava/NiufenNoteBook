...
@Component
@Slf4j
@RabbitListener(queues = FanoutRabbitMqConf.FANOUT_QUEUE_B)
public class FanoutReceiverB {
    ...
    public void process(SysUserEntity sysUserEntity) {
        log.error("FanoutReceiverC-userName:{}",sysUserEntity.getUsername());
        try {
            //  订阅类的应该自己做自己的业务操作
            sysUserService.listByCriteria(sysUserEntity);
        } catch (Exception e){
            e.printStackTrace();
        }
    }
