...
FROM (
	...
	FROM (
		...
		GROUP BY co1.city_id
	) co2
	LEFT JOIN (
		SELECT
			or1.order_city_id AS cityId,
			-- 配送费
		...
		--	AND or1.order_dep_id = 114901
			...
			AND dim.timeflag_month = '10'
		--	AND dim.timeflag_day = '28'
			GROUP BY or1.order_city_id
	) or2 on co2.cityId = or2.cityId
	LEFT JOIN lucky_operation.t_shop_info shopInfo ON shopInfo.department_id = co2.cityId
) a3