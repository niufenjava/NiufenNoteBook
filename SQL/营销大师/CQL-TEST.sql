...
SOURCE FlexQInput PROPERTIES ("zookeepers" = "10.104.105.90:5181","prefix" = "/meta", "topic" = "luckycrm_member_first_info_topic", "groupId" = "lucky_alg_channel_member")
--并发度2
parallel 2;
...
SINK FlexQOutput 
    PROPERTIES ("zookeepers" = "10.104.105.90:5181","prefix" = "/meta" ,"topic" = "lucky_analysis_channel_member")
--并发度2
parallel 2;
...
FROM market
(inviter__u__type is not null and inviter__u__code is not null and inviter__u__type=2)[RANGE 5 SECONDS BATCH]
...
FROM market
(inviter__u__type is not null and inviter__u__type!=2)[RANGE 5 SECONDS BATCH]
WHERE from_unixtime(created__u__time, 'yyyy-MM-dd') = from_unixtime(currenttimemillis(), 'yyyy-MM-dd') and
      first__u__login__u__time is null and
      ((from_unixtime(first__u__order__u__time, 'yyyy-MM-dd') = from_unixtime(currenttimemillis(), 'yyyy-MM-dd') and first__u__any__u__pay__u__time is null) or
      (from_unixtime(first__u__any__u__pay__u__time, 'yyyy-MM-dd') = from_unixtime(currenttimemillis(), 'yyyy-MM-dd') and first__u__order__u__time is null))
...
FROM market
(inviter__u__type=2 and inviter__u__code is not null)[RANGE 5 SECONDS BATCH] 
WHERE from_unixtime(created__u__time, 'yyyy-MM-dd') = from_unixtime(currenttimemillis(), 'yyyy-MM-dd') and 
      first__u__login__u__time is null and 
      first__u__order__u__time is null and 
      first__u__any__u__pay__u__time is null 
