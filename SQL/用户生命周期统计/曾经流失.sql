...
select * from (
    ...
    FROM
      ...
      LEFT JOIN (
                  ...
                  WHERE
                    o.STATUS = 50
                    AND o.dt > date_sub('2017-10-31', 30) -- 本月
                    AND o.dt <= '2017-10-31' -- 上月
    ...
    WHERE
      o.STATUS = 50
       -- 30天前下单用户
      AND o.dt <= date_sub('2017-10-31', 30)
      AND o.dep_id NOT IN (SELECT department_id FROM dw_dim.t_test_shop)
      AND m.status = 1
      AND m.id is not null
      -- 不包含30天内下单的用户
...
(
    ...
    FROM
      ...
      LEFT JOIN (
                  ...
                  WHERE
                    o.STATUS = 50
                    AND o.dt > date_sub('2017-11-30', 30) -- 本月
                    AND o.dt <= '2017-11-30' -- 上月
                    AND o.dep_id NOT IN (SELECT department_id FROM dw_dim.t_test_shop)
                    and m.status = 1
                    and m.id is not null
                ) h ON h.member_id = o.member_id
    ...
    WHERE
      o.STATUS = 50
       -- 30天前下单用户
      AND o.dt <= date_sub('2017-11-30', 30)
      AND o.dep_id NOT IN (SELECT department_id FROM dw_dim.t_test_shop)
      AND m.status = 1
      AND m.id is not null
      -- 不包含30天内下单的用户
